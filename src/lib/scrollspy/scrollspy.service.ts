...
import { UmaScrollspyGroup, SCROLLSPY_ANIMATION_SMOOTH } from './scrollspy';
import { Subscription, Observable, BehaviorSubject, fromEvent } from 'rxjs';
import { debounceTime, withLatestFrom } from 'rxjs/operators';

@Injectable()
export class UmaScrollspyService implements OnDestroy {
  /**
   ...
   */
  private data: UmaScrollspyGroup[] = [];
   ...
   * @param name string
   * @param items UmaScrollspyItemDirective[]
  ...
  create(
    ...
    let group: UmaScrollspyGroup = this.data.find(group => group.name === name);
    if (!group) {
      group = {
        name: name,
        animation: animation || SCROLLSPY_ANIMATION_SMOOTH,
        items: new BehaviorSubject<UmaScrollspyItemDirective[]>(items || []),
      };
   ...
   * @param name string
   */
  group(name: string): Observable<UmaScrollspyItemDirective[]> {
    let group: UmaScrollspyGroup = this.data.find(g => g.name === name);
    if (!group) {
  ...
  scrollTo(name: string, id: string): void {
    const group: UmaScrollspyGroup = this.data.find(group => group.name === name);
